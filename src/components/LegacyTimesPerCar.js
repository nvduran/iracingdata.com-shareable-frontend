...
export const LegacyTimesPerCar = ({ season, tracks }) => {
        const [isLoading, setIsLoading] = useState(true);
        const [carData, setCarData] = useState([]);

        useEffect(() => {
                const fetchData = async () => {
                        setIsLoading(true); // Reset loading state here
                        const allData = [];
                        for (const track of tracks) {
                                const response = await fetch(`https://iracing6-backend.herokuapp.com/api/legacy-times-by-car/seasons/${season}/tracks/${track}`);
        ...
        // Create the plots
        const plots = Object.entries(groupedCarData).map(([trackName, trackCarData]) => {
                ...
                //         const aIndex = filteredCarData.indexOf(a);
                //         const bIndex = filteredCarData.indexOf(b);
                //         return medianLapTimes[aIndex] - medianLapTimes[bIndex];
                // });
                ...
                const data = filteredCarData.map((car) => {
                        return {
                                ...
                                boxpoints: false,
                                marker: {
                                        color: carColors[car.car_name], // Set the color based on the car name
                                },
                        };
                });
        ...
        let reversedPlots = plots;

        // Reverse the order of the plots
        if (
                decodeURI(season) === "GT Sprint VRS Series - 2023 Season 3" ||
                decodeURI(season) === "GT3 Fanatec Challenge - Fixed - 2023 Season 3 - Fixed" ||
                decodeURI(season) === "GT4 Falken Tyre Challenge - 2023 Season 3 Fixed" ||
                decodeURI(season) === "GT4 Falken Tyre Challenge - 2023 Season 3" ||
                decodeURI(season) === "IMSA iRacing Series - 2023 Season 3" ||
                decodeURI(season) === "Touring Car Turn Racing Challenge - 2023 Season 3"
        ) {
                reversedPlots = plots.reverse();
        }
                ...
                return (
                        <div className="plots-container">
                                <div className="top-text-container">
                                        ...
                                        <h3 className="date-text">Last Updated: 6/10/2023</h3>
                                </div>
